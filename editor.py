...
class Editor(tk.Canvas):
    ...
    def __init__(self, root, level):
        ...
        for i, j in enumerate(bricks):
            ...
            self.create_rectangle(col*self.bricksWidth, line*self.bricksHeight, (col+1) *
                                  self.bricksWidth, (line+1)*self.bricksHeight, fill=color, width=2, outline="#ffffff")
    ...
    def setColor(self, id, color):
        ...
        for i in range(self.bricksNbByLine*self.linesNb):
            ...
            brickId = [id for id, color in self.bricksColors.items()
                       if color == brickColor]
            if brickId == []:
                content += "."
            else:
